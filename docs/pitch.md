...
> 整个项目实现了 6 个任务：14 个新 syscall、5 个内核工具、7 个 demo
> 程序、全部在 QEMU 上可演示。
...
> 我们做了 7 个 demo 程序，覆盖全部 6 个任务。

> 一键演示用 `agent_runner`，它会顺序跑完 6 个 demo：
>
> ```
> >> agent_runner
> ```
...
> - `agent_demo_file`（任务四）：跑 200 次属性查询，分别用 indexed
>   和 full-scan 路径，打印耗时对比。**这是性能评估数据。**
...
### Q11：你们 agent_demo_file 跑出来 "indexed is 1.00× faster"，是不是索引没起作用？

> **没起到加速作用，但起到了它该起的作用**——这是一个非常好的问题，
> 暴露了"算法复杂度"和"实测耗时"的本质区别。
>
> 三个原因：
> 1. **基线数据集只有 4 个文件**。全扫描 4 条记录是 L1 cache hit，开销不到 100 ns。
>    倒排索引的两次哈希查表 + SmallVec 求交集开销在同一数量级。
> 2. **QEMU `get_time_ms()` 来自 10 ms 一次的 timer 中断**。
>    200 次查询总共 11 ms，单次 55 µs 已经摸到时钟下限，分辨不出差距。
> 3. **倒排索引的本质优势是 O(k) vs O(N·c)**，必须靠 N 增长才能看到。
>    我在 perf-report §3.4 给出了理论扩展：N=100 时预计 240×，
>    N=10000 时预计 24000×。
>
> 重点：我把**机制下沉到内核**了——`build_index()` 在 PUT 时就花 O(c) 维护
> 倒排表，让 GET 时永远是 O(k)。这是 Hadoop / Elasticsearch / 任何一个生产
> 索引系统都用的设计，只是规模化时才显现。
>
> 我可以现场修改 `init_demo_attrs` 把数据集扩到 1000 条文件再跑一次给你看。
...
3. **`agent_demo_file`** —— 强调最后那行 "indexed is N.NN× faster"
